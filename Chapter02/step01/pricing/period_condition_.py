from constant_ import DAY_OF_WEEK
from typing import TYPE_CHECKING
from datetime import date, time, datetime
from discount_condition_ import AbsDiscountCondition

if TYPE_CHECKING:
    from screening_ import Screening


class PeriodCondition(AbsDiscountCondition):
    def __init__(
        self, day_of_week: DAY_OF_WEEK, start_time: time, end_time: time
    ) -> None:
        self.__day_of_week = day_of_week  # 조건에 쓸 요일
        self.__start_time = start_time  # 시작 시간
        self.__end_time = end_time  # 종료시간

    # 상영 요일이 dayOfWeek와 같고,
    # 상영시간이 startTime과 endTime 사이에 있느면 True
    def is_satisfied_by(self, screening: "Screening"):
        return (
            self.__day_of_week == screening.get_start_time().weekday()
            and self.__start_time <= screening.get_start_time().time()
            and self.__end_time >= screening.get_start_time().time()
        )

    def period_check(self):
        print(self.__day_of_week, self.__start_time, self.__end_time)
        # 0 09:00:00 13:00:00
