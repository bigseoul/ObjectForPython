[다형성]
영화에 따라 정책을 다르게 적용하는 방법
1. if-elif
-Enum으로 정책별 열거형 타입을 만든다. 
-이를 Movie 인스턴스 변수로 포함시킨다.
-정책별을 구분하는 분기문 만들어서, 정책을 적용한다.

2. 다형성 이용 1
-정책별 다른 Movie 타입 만들기
-Movie를 추상클래스로 만든다.
-할인 정책별 자식 클래스를 만든다.
-정책 적용할 부분만 자식에서 오버라이드 시킨다.

3. 다형성 이용 2
-정책 추상클래스를 만들고, 필요 정책 자식 클래스를 만든다.
-movie에 정책 클래스를 합성 시킨다. 예) Movie(adm: "AmountDiscountMovie")

[step04_1]
*정책을 따로 분리하고 합성하기
- AbsMovie를 다시 일반 Movie클래스로 바꾸기
--Movie인스턴스 변수 AbsDiscountPolicy: discount_policy 

- AbsDsicountPolicy 추상클래스 만들기
--추상메서드로:calculate_discount_amount()
- AmountDiscountPolicy/PercentDiscountPolicy/NoneDiscountPolicy 클래스 구체화
...continue
[step04]
*Movie 클래스 개선
1. 폴리모피즘/프로텍티드 베리에이션 적용
2. 금액 할인 정책과 비율 할인 정책, 그 이상의 정책이 있을 수 있음
3. Movie를 추상클래스로 만들고, 이를 AmountDiscountMovie/PercentDiscountMovie/NonDiscountMovie가 상속받음
-Movie의 인스턴스 변수 (title/running_time/fee/discount_conditions)
-Movie의 메서드 (calculate_movie_fee()/is_Discountable()/abs:calculate_discount_amount())
4. 공용로직/ 개별로직 
-추상메서드 calculate_discount_amount()
-discount_amount/ 
