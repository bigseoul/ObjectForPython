...
from invalid_screening_exception_ import InvalidScreeningException
...
class Movie:
    ...
    def calculate_movie_fee(self, screening: "Screening"):
        if screening == None or screening.get_start_time() < datetime.now():
            raise InvalidScreeningException()
        return self.__fee.minus(
            self.__discount_policy.calculate_discount_amount(screening)  # type: ignore
        )
    ...
    def get_end_time_from(self, when_started):
        return datetime.combine(when_started, self.__duration)
